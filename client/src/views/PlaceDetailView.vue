...
const categoryName = computed(() => {
	...
	return (
		CATEGORIES.find((c) => c.slug === place.value!.category)
			?.name ?? ''
	);
});
...
onMounted(async () => {
	try {
		const res = await api.get<{ data: Place }>(
			`/places/${props.id}`,
		);
		place.value = res.data;
	} catch (e) {
		error.value =
			e instanceof Error ? e.message : 'Failed to load place';
	} finally {
		loading.value = false;
	}
});
